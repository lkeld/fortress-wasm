...
pub fn op_pushstring(vm: &mut Vm) -> Result<bool, VmError> {
    ...
    for j in 0..len {
        ...
        let key_byte = vm.session_key[key_idx];
...
pub fn op_div(vm: &mut Vm) -> Result<bool, VmError> {
    ...
    match (a, b) {
        ...
        (Value::Float(a_val), Value::Float(b_val)) => {
            ...
            vm.stack.push(Value::Float(a_val / b_val))?;
        }
        _ => return Err(VmError::TypeError),
    }
    Ok(false)
...
pub fn op_length(vm: &mut Vm) -> Result<bool, VmError> {
    ...
    match target {
        Value::List(vec_rc) => {
            vm.stack.push(Value::Int(vec_rc.borrow().len() as i64))?;
        }
        Value::Str(s) => {
...
pub fn op_encryptaes(vm: &mut Vm) -> Result<bool, VmError> {
    ...
    if let (Value::Str(key_str), Value::Str(payload_str)) = (key_val, payload_val) {
        ...
        match crypto_core::encrypt_aes_gcm(payload_str.as_bytes(), &key_bytes) {
            ...
            Err(_) => return Err(VmError::TypeError),
        }
    } else {
        return Err(VmError::TypeError);
    }
