...
import { transpile } from '../js-transpiler';
...
export function verifyEquivalenceSync(
    ...
    try {
        originalFunc = vm.runInContext(`(function() { ${builtins}; return (${cleanJsCode}); })()`, context);
    } catch (e) {
        // Fallback for function declarations
        originalFunc = vm.runInContext(`(function() { ${builtins}; ${cleanJsCode}; return ${cleanJsCode.match(/function\s+(\w+)/)?.[1] || 'defaultFunc'}; })()`, context);
    }
...
export async function verifyEquivalence(
    ...
    let originalFunc: Function;
    const origSandbox = {
        Symbol: (desc: any) => "__fortress_sym_" + desc,
        console,
        JSON,
        Math,
        String,
    ...
    const origContext = vm.createContext(origSandbox);
    try {
        originalFunc = vm.runInContext(`(function() { ${builtins}; return (${cleanJsCode}); })()`, origContext);
    } catch (e) {
        originalFunc = vm.runInContext(`(function() { ${builtins}; ${cleanJsCode}; return ${cleanJsCode.match(/function\s+(\w+)/)?.[1] || 'defaultFunc'}; })()`, origContext);
    }
